...
test('home page renders configured shell and recent content', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: /KansasPattons home/i })).toBeVisible();
...
test('search page returns clickable cross-type results', async ({ page }) => {
  ...
  await page.getByRole('searchbox', { name: 'Term' }).fill('breakfast');
  await page.getByRole('button', { name: 'Search' }).click();

  await expect(page).toHaveURL(/\/search\?q=breakfast/);
